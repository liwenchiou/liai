...
> 你被強制要求必須先使用你的「檔案讀取工具（如 view_file, read_file 或檔案庫檢索）」去讀取專案根目錄下的 `.agent` 檔案。
> 在你沒有實際執行讀取動作前，嚴禁進行任何後續動作！
> **(防呆容錯)**：若發現該檔案不存在，即代表該專案無專屬規則，請直接以本全域大腦為主繼續執行任務，絕不可因此報錯中斷；若檔案存在且內容與全域大腦衝突，則一律以專案本地的 `.agent` 為主。
