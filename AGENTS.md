...
由於 `.agents` (即 LIAI 大腦) 屬於本地端的開發輔助工具，**絕不可將其污染至個別專案的原始碼與部署環境中**。
- **自動忽略**：當你接手任何掛載了 `.agents` 的專案時，必須主動檢查專案根目錄的 `.gitignore` 與 `.dockerignore`。
- **阻絕封裝**：若發現 `.agents` 尚未被忽略，請主動提醒我，或產生指令將 `.agents` 寫入這兩個忽略清單中，確保 LIAI 大腦絕對不會被意外 Commit 進入版控，也絕對不會被打包進 Docker Image 裡。
